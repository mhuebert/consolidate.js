 ...
 * Serenade support,
 */
exports.serenade = fromStringRenderer('serenade')


/**
 * Serenade string support,
 */

exports.serenade.render = function(str, options, fn){
  var serenade = requires.serenade || requires.serenade = require('serenade')
  var Serenade = serenade.Serenade
  var document = requires.document || requires.document = require('jsdom').jsdom(null, null, {})
  var window = requires.window || requires.window = document.createWindow()
  Serenade.document = document
  try {
    var element = Serenade.view(templateString).render(model, {})
    document.body.appendChild(element)
    fn(null, document.body.innerHTML)
  } catch (err) {
    fn(err)
  }
}
